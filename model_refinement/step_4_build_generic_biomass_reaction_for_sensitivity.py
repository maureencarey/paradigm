...
logging.basicConfig(filename='step4_for_sensivity_{}_{}.log'.format(SPECIES_ID,day), level=logging.INFO, filemode='w')
...
# load curated models
os.chdir(model_path)
chominis = cobra.io.read_sbml_model('published/chominis_vanee2010.xml')
tgondii = cobra.io.read_sbml_model('published/tg_tymoshenko2015.xml')
leish = cobra.io.read_sbml_model('published/iAC560_leishmania_Chavali2008.xml')
...
os.chdir(data_path)
df.to_csv('./results/biomass_components_for_sensitivity_{}.csv'.format(str(date.today())), header=True, index=True)
...
cobra.io.save_json_model(model, "for_sensitivity_with_biomass_"+SPECIES_ID+".json")
cobra.io.write_sbml_model(model, "for_sensitivity_with_biomass_"+SPECIES_ID+".xml")
