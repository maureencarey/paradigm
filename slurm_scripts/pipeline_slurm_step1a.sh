...
diamond blastp -d ~/paradigm/data/bigg_proteins_diamond -q $filename -o "${filename:2:${#filename}-36}_BiGG.tsv"
done
...
cd ~/paradigm/data/genomes/protein
for filename in ./*_annotatedProteins_March2019.fasta; do
diamond blastp -d ~/paradigm/data/orthoMCL_proteins_diamond -q $filename -o "${filename:2:${#filename}-36}_orthoMCL.tsv"
done
