...
for filename in ./diamond_output_BiGG/*_BiGG.tsv; do
    ...
    foo=${species_string}"_TEST.sbatch"
    cp slurm_template.slurm $foo
    echo "python3 ../denovo_reconstruction/step_3_build_de_novo_models_TEST.py '$file_without_pre'" >> $foo
    echo "python3 ../model_refinement/step_4_build_generic_biomass_reaction_TEST.py 'final_denovo_$species_string.json'" >> $foo #
    if [[ " $plasmodium_list " =~ .*\ $species_string\ .* ]]; then
    echo "python3 ../model_refinement/step_5_orthology_based_semi_curation_TEST.py 'with_biomass_$species_string.json'" >> $foo
    echo "python3 ../model_refinement/step_6_task_based_gapfilling_TEST.py 'ortho_$species_string.json'" >> $foo
    else
